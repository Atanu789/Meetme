...
const meetingId = process.argv[2] || 'caption-test-moihjc81';
const testAudioUrl = process.env.ASSEMBLYAI_TEST_AUDIO_URL || 'https://storage.googleapis.com/aai-web-samples/5_common_sports_injuries.mp3';
...
function extractCaptionSegments(transcript) {
  ...
  return text ? [{ text, speaker: 'AssemblyAI Test' }] : [];
}

async function postCaption(text, speaker = 'AssemblyAI Test') {
  const payload = {
    text,
    speaker,
    final: true,
...
(async () => {
  ...
  console.log(`[test] Transcribing audio from: ${testAudioUrl}`);
  const transcript = await transcribeRemoteAudio(testAudioUrl);
  ...
  console.log(`[test] Transcript OK (${String(transcript.text || '').length} chars)`);
  for (const segment of segments.slice(0, 4)) {
    await postCaption(segment.text.slice(0, 240), segment.speaker);
  }
  console.log(`[test] Caption posted to meeting: ${meetingId}`);
})().catch((error) => {
