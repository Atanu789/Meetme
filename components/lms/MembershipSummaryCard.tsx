      ...
      <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
        <div className="min-w-[13rem]">
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.16em] text-[#a6a1a6]"><LockKeyhole className="h-4 w-4 text-[#ef233c]" />Membership status</div>
            <span className="rounded-full border border-[#ef233c]/60 bg-[#ef233c]/10 px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-[#ff9ba7]">Signed in</span>
          </div>
          <p className="mt-5 text-[11px] font-bold uppercase tracking-[0.16em] text-[#7f7b80]">Current workspace</p>
          <h2 className="mt-1 font-display text-3xl font-semibold tracking-[-0.04em] text-white sm:text-4xl">{workspace.title}</h2>
          <p className="mt-1 text-sm text-[#b6b1b5]">{activeUntil} &middot; {credits}</p>
        </div>
        ...
        <dl className="grid flex-1 divide-y divide-white/10 border-y border-white/10 sm:grid-cols-2 sm:divide-y-0 sm:divide-x xl:grid-cols-4">
          <div className="min-w-0 py-3 sm:px-4 sm:first:pl-0 xl:py-1"><dt className="flex items-center gap-2 text-xs text-[#aaa5aa]"><Video className="h-4 w-4 shrink-0 text-[#ef233c]" />Meeting cap</dt><dd className="mt-2 truncate text-sm font-bold text-white">{workspace.limits.maxParticipants ?? 'Custom'} people &middot; {formatDuration(workspace.limits.maxMeetingMinutes)}</dd></div>
          <div className="min-w-0 py-3 sm:px-4 xl:py-1"><dt className="flex items-center gap-2 text-xs text-[#aaa5aa]"><Users className="h-4 w-4 shrink-0 text-[#ef233c]" />Workspace seats</dt><dd className="mt-2 text-sm font-bold text-white">{seatLabel}</dd></div>
          <div className="min-w-0 py-3 sm:px-4 sm:pt-4 xl:py-1"><dt className="flex items-center gap-2 text-xs text-[#aaa5aa]"><HardDrive className="h-4 w-4 shrink-0 text-[#ef233c]" />Storage</dt><dd className="mt-2 truncate text-sm font-bold text-white">{storageLabel}</dd></div>
          <div className="min-w-0 py-3 sm:px-4 sm:pt-4 xl:py-1"><dt className="flex items-center gap-2 text-xs text-[#aaa5aa]"><CreditCard className="h-4 w-4 shrink-0 text-[#ef233c]" />Billing</dt><dd className="mt-2 text-sm font-bold text-white">INR</dd></div>
        </dl>
        ...
        <div className="flex shrink-0 flex-col items-start gap-3 xl:items-end">
          ...
          <p className="max-w-[15rem] text-xs leading-5 text-[#777277] xl:text-right">Seats are workspace members. Meeting participants are counted separately.</p>
        </div>
      </div>
    </section>
  );
