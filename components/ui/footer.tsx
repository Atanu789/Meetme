    ...
    <footer className="border-t border-[#2a3039] bg-[#0b0d10]">
      <div className="mx-auto grid max-w-[80rem] gap-8 px-4 py-10 sm:px-5 sm:grid-cols-2 lg:grid-cols-[1.6fr_repeat(3,1fr)]">
        <div>
          ...
          <Link href="/sign-up" className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-[#37d7ff] hover:text-[#6be3ff]">
            Get started
          ...
          <div key={section.title}>
            ...
            <ul className="mt-4 space-y-2.5 text-sm">
              {section.links.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-[#c7d0d9] hover:text-[#37d7ff]">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
